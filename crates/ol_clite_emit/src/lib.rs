...
pub fn emit_project(project: &Project) -> EmittedClite {
    ...
    // Float intrinsics (sqrt, sin, fmin, …) call the double `<math.h>`
    // family; the include is unconditional so generation stays deterministic.
    let _ = writeln!(header, "#include <math.h>");
    header.push('\n');
...
fn expr_type(expr: &Expr, ctx: &EmitCtx) -> Option<Type> {
    match expr {
        ...
        Expr::FloatIntrinsic { single, .. } => {
            Some(if *single { Type::Float32 } else { Type::Float64 })
        }
        Expr::Call { node, .. } => ctx
            .project
            .all_nodes()
