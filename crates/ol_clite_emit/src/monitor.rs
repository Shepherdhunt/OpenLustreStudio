...
pub fn emit_monitors(project: &Project) -> MonitorBundle {
    ...
    let _ = writeln!(source, "#include <assert.h>");
    source.push('\n');
...
fn emit_monitor(c: &ContractDef, owner: &str, header: &mut String, source: &mut String) {
    ...
    let _ = writeln!(
        header,
        "void {monitor_name}_check({monitor_name}_State* s, const {owner}_Input* in, const {owner}_Output* out);"
    );
    ...
    for (i, a) in c.assumptions.iter().enumerate() {
        ...
        let _ = writeln!(
            source,
            "  if (!({cond})) {{ s->any_violation = true; /* {label} violated */ }}"
        );
    }
    ...
    for (idx, m) in c.modes.iter().enumerate() {
        ...
        let _ = writeln!(source, "    s->active_mode = {idx}; /* mode {} */", m.name);
        for (j, e) in m.ensures.iter().enumerate() {
            let cond = emit_mon_expr(e, &scope);
            let _ = writeln!(
                source,
                "    if (!({cond})) {{ s->any_violation = true; /* {}::ensure#{j} violated */ }}",
                m.name
            );
        }
        let _ = writeln!(source, "  }}");
