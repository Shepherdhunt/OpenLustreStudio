...
// Generated by build.rs: the on-disk `libraries/` tree captured at compile
// time as `EMBEDDED_LIBRARIES: &[(&str, &str)]`.
include!(concat!(env!("OUT_DIR"), "/embedded_libraries.rs"));

/// Load the standard library that was embedded into this binary at compile
/// time — the deployed-app path, where no `libraries/` checkout exists on
/// disk. Identical parsing and lowering to [`load_dir`].
pub fn load_embedded() -> Result<Library, LoadError> {
    let mut lib = Library::default();
    for (rel_path, contents) in EMBEDDED_LIBRARIES {
        let raw = parse_blocks(contents)
            .map_err(|e| LoadError::Yaml(PathBuf::from(rel_path), e))?;
        for b in raw {
            let lowered = b.lower()?;
            lib.entries.push(LibraryEntry {
                source: PathBuf::from(format!("<embedded>/{rel_path}")),
                block: lowered,
            });
        }
    }
