...
fn doctor_report() -> String {
    ...
    let _ = writeln!(s, "        Enables: emulated-target runs (`clite-emulate`, incl. `--system`) — the");
    let _ = writeln!(s, "                 third equivalence backend (cross-compile + run under QEMU).");
    match (docker.as_deref(), daemon) {
        (None, _) => {
...
pub(crate) fn makefile_for_entry(entry: &str) -> String {
    // The host profile reproduces the original generic Makefile; named targets
    // (embedded Linux, VxWorks, bare-metal) go through `makefile_for_target`.
    crate::target::makefile_for_target(entry, &crate::target::find_target(Some("host")))
}
