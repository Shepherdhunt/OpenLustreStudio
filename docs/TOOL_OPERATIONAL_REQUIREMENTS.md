...
| TOR-411 | The tool SHALL emit a self-contained Docker context that cross-compiles the generated C-Lite for armhf and runs it under QEMU user-mode, and (where Docker is available) SHALL build + run it and verify its trace against the IR simulator cell-for-cell — a third equivalence backend. Where Docker is absent it SHALL emit the harness + the IR-sim reference + run instructions. | T (Docker-gated for the live run) | `crates/ol_cli/src/emulate.rs` unit tests (`dockerfile_cross_compiles_and_runs_under_qemu`, `traces_match_on_shared_columns`, `traces_mismatch_is_located`, …); `openlustre clite-emulate` |
