...
| TOR-411 | The tool SHALL emit a self-contained Docker context that cross-compiles the generated C-Lite and runs it under QEMU — user-mode (armhf) by default, or full-system (`--system`: `qemu-system-aarch64 -M virt` + a busybox initramfs, kernel supplied as input) — and (where Docker is available) SHALL build + run it and verify its trace against the IR simulator cell-for-cell, a third equivalence backend. Where Docker is absent it SHALL emit the harness + the IR-sim reference + run instructions. | T (Docker-gated for the live run) | `crates/ol_cli/src/emulate.rs` unit tests (`dockerfile_cross_compiles_and_runs_under_qemu`, `system_dockerfile_boots_qemu_virt_with_a_user_kernel`, `extract_framed_trace_pulls_the_csv_out_of_boot_noise`, `traces_*`); `openlustre clite-emulate [--system]` |
